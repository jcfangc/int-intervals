...
mod basic {
    ...
    impl U16CO {
        ...
        /// Construct a `U16CO` from a start position and a length.
        ///
        /// The resulting interval is `[start, start + len)`.
        ///
        /// Returns `None` if:
        /// - `len == 0`;
        /// - `start + len` overflows `u16`.
        #[inline]
        pub const fn checked_from_start_len(start: u16, len: u16) -> Option<Self> {
            if len == 0 {
                return None;
            }
        ...
        /// Returns `None` if:
        /// - `len == 0`;
        /// - saturation still produces an empty interval, e.g. `start == u16::MAX`.
        #[inline]
        pub const fn saturating_from_start_len(start: u16, len: u16) -> Option<Self> {
            if len == 0 {
                return None;
            }
        ...
        pub const fn checked_from_midpoint_len(mid: u16, len: u16) -> Option<Self> {
            ...
            // 1. `mid.checked_sub(len / 2)` ensures `start` does not underflow `u16`.
            // 2. `start.checked_add(len)` ensures `end_excl` does not overflow `u16`.
    ...
    impl U16CO {
        ...
        #[inline]
        pub const fn end_incl(self) -> u16 {
            // u16_low_bound =< start < end_excl
            self.end_excl - 1
        }
    }
    ...
    impl U16CO {
        #[inline]
        pub const fn iter(self) -> core::ops::Range<u16> {
            self.start..self.end_excl
        }

        #[inline]
        pub const fn to_range(self) -> core::ops::Range<u16> {
            self.start..self.end_excl
        }
    }
}
...
// ------------------------------------------------------------
// Module: Minkowski arithmetic for U16CO
    ...
    pub mod checked {
        ...
        impl U16CO {
            ...
            pub const fn checked_minkowski_mul_hull(self, other: Self) -> Option<Self> {
                ...
                // SAFETY:
                // For `U16CO`, all values are non-negative, so endpoint-wise multiplication
...
crate::interval::traits::impl_co_forwarding!(U16CO, u16, u16);
