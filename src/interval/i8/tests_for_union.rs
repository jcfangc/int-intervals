...
mod unit_tests {
    ...
    fn span(lo: i8, hi: i8) -> I8CO {
        I8CO::try_new(lo, hi).unwrap()
    }
...
mod prop_tests {
    ...
    fn span(a: i8, b: i8) -> Option<I8CO> {
        let lo = a.min(b);
        let hi = a.max(b);
        I8CO::try_new(lo, hi)
    }

    fn edge_values() -> Vec<i8> {
        let mut v = vec![i8::MIN, i8::MAX, 0, 1];

        if i8::MIN < i8::MAX {
            v.push(i8::MIN.saturating_add(1));
            v.push(i8::MAX.saturating_sub(1));
        }
    ...
    fn edge_scalar() -> impl Strategy<Value = i8> {
    ...
    fn span_strategy() -> impl Strategy<Value = I8CO> {
        (mixed_scalar(), mixed_scalar()).prop_filter_map("non-empty interval", |(a, b)| span(a, b))
    }

    fn union_contains(u: OneTwo<I8CO>, p: i8) -> bool {
        match u {
        ...
        fn union_shape_is_correct(
            ...
            match x.union(y) {
                ...
                OneTwo::One(z) => {
                    ...
                    prop_assert_eq!(z, I8CO::try_new(start, end_excl).unwrap());
                }
