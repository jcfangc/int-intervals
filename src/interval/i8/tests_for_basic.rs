...
mod unit_tests {
    ...
    mod start_len_api {
        ...
        const LEN_ONE: u8 = 1;
        const LEN_TWO: u8 = 2;
        const LEN_SMALL: u8 = 5;
        const LEN_WIDE: u8 = 10;

        const POS_ROOM_FROM_ZERO: u8 = MAX as u8;
        const ZERO_CROSS_LEN_FROM_MIN: u8 = POS_ROOM_FROM_ZERO + LEN_ONE;
        const MAX_INTERVAL_LEN: u8 = u8::MAX;

        #[test]
        fn checked_from_start_len_basic() {
            let iv = I8CO::checked_from_start_len(ZERO, LEN_SMALL).unwrap();
            assert_eq!(iv.start(), ZERO);
            assert_eq!(iv.end_excl(), LEN_SMALL as i8);
            assert_eq!(iv.len(), LEN_SMALL);

            let start = -5;
            let iv = I8CO::checked_from_start_len(start, LEN_SMALL).unwrap();
            assert_eq!(iv.start(), start);
            assert_eq!(iv.end_excl(), ZERO);
            assert_eq!(iv.len(), LEN_SMALL);

            let iv = I8CO::checked_from_start_len(start, LEN_WIDE).unwrap();
            assert_eq!(iv.start(), start);
            assert_eq!(iv.end_excl(), LEN_SMALL as i8);
        ...
        fn checked_from_start_len_rejects_zero_len() {
            assert!(I8CO::checked_from_start_len(ZERO, 0).is_none());
            assert!(I8CO::checked_from_start_len(MIN, 0).is_none());
            assert!(I8CO::checked_from_start_len(MAX, 0).is_none());
        }

        #[test]
        fn checked_from_start_len_handles_negative_side() {
            let iv = I8CO::checked_from_start_len(MIN, LEN_ONE).unwrap();
            assert_eq!(iv.start(), MIN);
            assert_eq!(iv.end_excl(), MIN + ONE);
            assert_eq!(iv.len(), LEN_ONE);

            let iv = I8CO::checked_from_start_len(MIN, LEN_WIDE).unwrap();
            assert_eq!(iv.start(), MIN);
            assert_eq!(iv.end_excl(), MIN + LEN_WIDE as i8);
            assert_eq!(iv.len(), LEN_WIDE);

            let iv = I8CO::checked_from_start_len(NEG_ONE, LEN_ONE).unwrap();
        ...
        fn checked_from_start_len_handles_cross_zero() {
            let iv = I8CO::checked_from_start_len(NEG_ONE, LEN_TWO).unwrap();
            assert_eq!(iv.start(), NEG_ONE);
            ...
            let start = -10;
            let len = LEN_WIDE * 2;
            let iv = I8CO::checked_from_start_len(start, len).unwrap();
            assert_eq!(iv.start(), start);
            assert_eq!(iv.end_excl(), 10);
            assert_eq!(iv.len(), len);

            let iv = I8CO::checked_from_start_len(MIN, ZERO_CROSS_LEN_FROM_MIN).unwrap();
            assert_eq!(iv.start(), MIN);
            assert_eq!(iv.end_excl(), ZERO);
        ...
        fn checked_from_start_len_accepts_maximal_interval() {
            let iv = I8CO::checked_from_start_len(MIN, MAX_INTERVAL_LEN).unwrap();
            assert_eq!(iv.start(), MIN);
        ...
        fn checked_from_start_len_rejects_positive_overflow() {
            assert!(I8CO::checked_from_start_len(ZERO, POS_ROOM_FROM_ZERO + LEN_ONE).is_none());

            assert!(I8CO::checked_from_start_len(ONE, POS_ROOM_FROM_ZERO).is_none());
            assert!(I8CO::checked_from_start_len(MAX - ONE, LEN_TWO).is_none());
            assert!(I8CO::checked_from_start_len(MAX, LEN_ONE).is_none());
        }

        #[test]
        fn checked_from_start_len_rejects_cross_zero_overflow() {
            let from_neg_one_overflow_len = POS_ROOM_FROM_ZERO + LEN_TWO;
            assert!(I8CO::checked_from_start_len(NEG_ONE, from_neg_one_overflow_len).is_none());

            let start = -10;
            let to_zero = (-start) as u8;
            let overflow_len = to_zero + POS_ROOM_FROM_ZERO + LEN_ONE;
            assert!(I8CO::checked_from_start_len(start, overflow_len).is_none());
        }

        #[test]
        fn saturating_from_start_len_basic() {
            let iv = I8CO::saturating_from_start_len(ZERO, LEN_SMALL).unwrap();
            assert_eq!(iv.start(), ZERO);
            assert_eq!(iv.end_excl(), LEN_SMALL as i8);
            assert_eq!(iv.len(), LEN_SMALL);

            let start = -5;
            let iv = I8CO::saturating_from_start_len(start, LEN_WIDE).unwrap();
            assert_eq!(iv.start(), start);
            assert_eq!(iv.end_excl(), LEN_SMALL as i8);
            assert_eq!(iv.len(), LEN_WIDE);
        }
        ...
        fn saturating_from_start_len_rejects_zero_len() {
            assert!(I8CO::saturating_from_start_len(ZERO, 0).is_none());
            assert!(I8CO::saturating_from_start_len(MIN, 0).is_none());
            assert!(I8CO::saturating_from_start_len(MAX, 0).is_none());
        }

        #[test]
        fn saturating_from_start_len_clamps_positive_overflow() {
            let iv = I8CO::saturating_from_start_len(ZERO, POS_ROOM_FROM_ZERO + LEN_ONE).unwrap();
            assert_eq!(iv.start(), ZERO);
            assert_eq!(iv.end_excl(), MAX);
            assert_eq!(iv.len(), POS_ROOM_FROM_ZERO);

            let iv = I8CO::saturating_from_start_len(ONE, POS_ROOM_FROM_ZERO).unwrap();
            assert_eq!(iv.start(), ONE);
            assert_eq!(iv.end_excl(), MAX);
            assert_eq!(iv.len(), POS_ROOM_FROM_ZERO - LEN_ONE);

            let iv = I8CO::saturating_from_start_len(MAX - ONE, LEN_TWO).unwrap();
        ...
        fn saturating_from_start_len_clamps_cross_zero_overflow() {
            ...
            let iv = I8CO::saturating_from_start_len(NEG_ONE, from_neg_one_overflow_len).unwrap();
            assert_eq!(iv.start(), NEG_ONE);
            assert_eq!(iv.end_excl(), MAX);
            assert_eq!(iv.len(), POS_ROOM_FROM_ZERO + LEN_ONE);
            ...
            let iv = I8CO::saturating_from_start_len(start, overflow_len).unwrap();
            assert_eq!(iv.start(), start);
            assert_eq!(iv.end_excl(), MAX);
        ...
        fn saturating_from_start_len_rejects_empty_after_saturation() {
            assert!(I8CO::saturating_from_start_len(MAX, LEN_ONE).is_none());
            assert!(I8CO::saturating_from_start_len(MAX, MAX_INTERVAL_LEN).is_none());
        }

        #[test]
        fn saturating_from_start_len_accepts_maximal_interval() {
            let iv = I8CO::saturating_from_start_len(MIN, MAX_INTERVAL_LEN).unwrap();
            assert_eq!(iv.start(), MIN);
            assert_eq!(iv.end_excl(), MAX);
            assert_eq!(iv.len(), MAX_INTERVAL_LEN);
        }
        ...
        fn checked_from_start_len_matches_try_new_when_reconstructing() {
            ...
            for (start, end_excl) in cases {
                let base = I8CO::try_new(start, end_excl).unwrap();
                let rebuilt = I8CO::checked_from_start_len(start, base.len()).unwrap();
...
mod prop_tests {
    ...
    mod start_len_api {
        ...
        const LEN_ZERO: u8 = 0;
        const LEN_ONE: u8 = 1;
        const LEN_TWO: u8 = LEN_ONE + LEN_ONE;

        const POS_ROOM_FROM_ZERO: u8 = MAX as u8;
        const ZERO_CROSS_LEN_FROM_MIN: u8 = POS_ROOM_FROM_ZERO + LEN_ONE;
        const MAX_INTERVAL_LEN: u8 = u8::MAX;

        fn edge_starts() -> Vec<i8> {
        ...
        fn mixed_start() -> impl Strategy<Value = i8> {
            prop_oneof![
        ...
        fn mixed_len() -> impl Strategy<Value = u8> {
            prop_oneof![
        ...
        proptest! {
            ...
            fn prop_checked_from_start_len_preserves_start_and_len(
                ...
                if let Some(iv) = I8CO::checked_from_start_len(start, len) {
            ...
            fn prop_checked_from_start_len_reconstructs_existing_interval(
                ...
                if let Some(base) = I8CO::try_new(start, end_excl) {
                    let rebuilt = I8CO::checked_from_start_len(base.start(), base.len());
            ...
            fn prop_checked_from_start_len_rejects_zero_len(start in mixed_start()) {
                prop_assert_eq!(I8CO::checked_from_start_len(start, LEN_ZERO), None);
            }
            ...
            fn prop_saturating_from_start_len_preserves_invariant(
                ...
                if let Some(iv) = I8CO::saturating_from_start_len(start, len) {
                    prop_assert_eq!(iv.start(), start);
                    prop_assert!(iv.start() < iv.end_excl());
            ...
            fn prop_saturating_from_start_len_matches_checked_when_checked_succeeds(
                ...
                let checked = I8CO::checked_from_start_len(start, len);
                let saturated = I8CO::saturating_from_start_len(start, len);
            ...
            fn prop_saturating_from_start_len_rejects_zero_len(start in mixed_start()) {
                prop_assert_eq!(I8CO::saturating_from_start_len(start, LEN_ZERO), None);
            }
