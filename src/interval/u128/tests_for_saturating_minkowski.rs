...
mod unit_tests {
    ...
    fn test_saturating_minkowski_add_basic() {
        let a = U128CO::try_new(1, 5).unwrap();
        let b = U128CO::try_new(2, 4).unwrap();
    ...
    fn test_saturating_minkowski_sub_basic() {
        let a = U128CO::try_new(5, 10).unwrap();
        let b = U128CO::try_new(2, 4).unwrap();
    ...
    fn test_saturating_minkowski_mul_basic() {
        let a = U128CO::try_new(1, 4).unwrap();
        let b = U128CO::try_new(2, 3).unwrap();
    ...
    fn test_saturating_minkowski_div_basic() {
        let a = U128CO::try_new(4, 10).unwrap();
        let b = U128CO::try_new(2, 5).unwrap();
    ...
    fn test_saturating_sub_n_can_collapse_to_none() {
        let a = U128CO::try_new(1, 3).unwrap();
...
mod prop_tests {
    ...
    fn interval_strategy() -> impl Strategy<Value = U128CO> {
        prop_oneof![
            Just(U128CO::try_new(u128::MIN, u128::MIN + 1).unwrap()), // [0,1)
            Just(U128CO::try_new(u128::MIN, u128::MAX).unwrap()),     // [0,255)
            Just(U128CO::try_new(u128::MAX - 1, u128::MAX).unwrap()), // [254,255)
            (u128::MIN..=u128::MAX, u128::MIN..=u128::MAX)
                .prop_filter_map("valid interval", |(s, e)| U128CO::try_new(s, e))
        ]
    }

    fn scalar_strategy() -> impl Strategy<Value = u128> {
        prop_oneof![Just(0), Just(1), Just(u128::MAX), any::<u128>()]
    }

    #[inline]
    fn endpoints(x: U128CO) -> [u128; 2] {
        [x.start(), x.end_incl()]
    }
