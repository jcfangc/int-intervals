...
mod unit_tests {
    ...
    fn test_minkowski_mul_basic() {
        let a = UsizeCO::try_new(1, 4).unwrap(); // [1,4)
        let b = UsizeCO::try_new(2, 3).unwrap(); // [2,3)
    ...
    fn test_minkowski_overflow() {
        let a = UsizeCO::try_new(250, usize::MAX).unwrap();
        let b = UsizeCO::try_new(10, 20).unwrap();
...
mod prop_tests {
    ...
    fn interval_strategy() -> impl Strategy<Value = UsizeCO> {
        prop_oneof![
            Just(UsizeCO::try_new(usize::MIN, usize::MIN + 1).unwrap()), // [0,1)
            Just(UsizeCO::try_new(usize::MIN, usize::MAX).unwrap()),     // [0,255)
            Just(UsizeCO::try_new(usize::MAX - 1, usize::MAX).unwrap()), // [254,255)
