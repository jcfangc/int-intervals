...
mod unit_tests {
    ...
    fn span(a: isize, b: isize) -> IsizeCO {
        IsizeCO::try_new(a, b).unwrap()
    }
...
mod prop_tests {
    ...
    fn span(a: isize, b: isize) -> Option<IsizeCO> {
        let lo = a.min(b);
        let hi = a.max(b);
        IsizeCO::try_new(lo, hi)
    }

    fn edge_values() -> Vec<isize> {
        let mut v = vec![isize::MIN, isize::MAX, 0, 1];

        if isize::MIN < isize::MAX {
            v.push(isize::MIN.saturating_add(1));
            v.push(isize::MAX.saturating_sub(1));
        }
    ...
    fn contains_zot(xs: ZeroOneTwo<IsizeCO>, p: isize) -> bool {
        match xs {
