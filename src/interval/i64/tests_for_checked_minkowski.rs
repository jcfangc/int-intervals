...
mod unit_tests {
    ...
    fn test_minkowski_add_basic() {
        let a = I64CO::try_new(1, 5).unwrap();
        let b = I64CO::try_new(-3, 2).unwrap();
    ...
    fn test_minkowski_sub_basic() {
        let a = I64CO::try_new(5, 10).unwrap();
        let b = I64CO::try_new(2, 4).unwrap();
    ...
    fn test_minkowski_mul_basic() {
        let a = I64CO::try_new(-2, 3).unwrap(); // [-2,3)
        let b = I64CO::try_new(-1, 2).unwrap(); // [-1,2)
    ...
    fn test_minkowski_div_basic() {
        let a = I64CO::try_new(-4, 10).unwrap();
        let b = I64CO::try_new(2, 5).unwrap();
    ...
    fn test_minkowski_div_by_zero() {
        let a = I64CO::try_new(1, 5).unwrap();
        let b = I64CO::try_new(0, 3).unwrap();
...
mod prop_tests {
    ...
    fn interval_strategy() -> impl Strategy<Value = I64CO> {
        prop_oneof![
            Just(I64CO::try_new(i64::MIN, i64::MIN + 1).unwrap()), // [-128, -127)
            Just(I64CO::try_new(i64::MIN, i64::MAX).unwrap()),     // [-128, 127)
            Just(I64CO::try_new(i64::MAX - 1, i64::MAX).unwrap()), // [126, 127)
            (i64::MIN..=i64::MAX, i64::MIN..=i64::MAX)
                .prop_filter_map("valid interval", |(s, e)| I64CO::try_new(s, e))
        ]
    }
