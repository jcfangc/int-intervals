...
mod basic {
    ...
    #[inline]
    const fn checked_end_excl_from_start_len(start: i8, len: u8) -> Option<i8> {
        end_excl_from_start_len(start, len, false)
    }

    #[inline]
    const fn saturating_end_excl_from_start_len(start: i8, len: u8) -> Option<i8> {
    ...
    #[inline]
    const fn end_excl_from_start_len(start: i8, len: u8, saturating: bool) -> Option<i8> {
        if len == 0 {
            return None;
        }

        if start < 0 {
            let to_zero = if start == i8::MIN {
                (i8::MAX as u8) + 1
            } else {
                (-start) as u8
            };
            ...
            if len < to_zero {
                let Some(end_excl) = start.checked_add(len as i8) else {
                    return None;
                };
                Some(end_excl)
            } else if len == to_zero {
                ...
                if rem > i8::MAX as u8 {
                    if saturating { Some(i8::MAX) } else { None }
                } else {
                    Some(rem as i8)
                }
            }
        } else {
            let room = (i8::MAX - start) as u8;
    ...
    impl I8CO {
        ...
        /// Returns `None` if:
        /// - `len == 0`;
        /// - saturation still produces an empty interval, e.g. `start == i8::MAX`.
        #[inline]
...
crate::interval::traits::impl_co_forwarding!(I8CO, i8, u8);
